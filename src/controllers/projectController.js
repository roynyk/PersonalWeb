...
export async function createProject(req, res, db) {
    try {
        ...
        const projectQuery = `
            ...
            RETURNING id
        `;
        const projectResult = await db.query(projectQuery, [1, projectName, startDate, endDate, description]);
        ...
        if (technologies) {
            ...
            for (let i = 0; i < technologies.length; i++) {
                ...
                if (techId) {
                    const relationQuery = "INSERT INTO project_technologies (project_id, technology_id) VALUES ($1, $2)";
...
export async function getEditProject(req, res, db) {
    try {
        ...
        const flash = req.session.flash;
        delete req.session.flash;
        res.render("edit-project", { title: "Edit Project", project: project, flash: flash });
    } catch (error) {
...
export async function updateProject(req, res, db) {
    try {
        ...
        const updateQuery = `
            ...
            WHERE id = $5
        `;
        await db.query(updateQuery, [projectName, startDate, endDate, description, id]);

        // 2. Hapus semua teknologi lama yang nempel di tabel project_technologies
        const deleteTechQuery = "DELETE FROM project_technologies WHERE project_id = $1";
        await db.query(deleteTechQuery, [id]);

        // 3. Masukkan teknologi yang baru dicentang user (Mirip seperti createProject)
        if (technologies) {
            if (typeof technologies === 'string') {
                technologies = [technologies];
            }
